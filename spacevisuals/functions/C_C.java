...
public class C_C{
     ...
     */
    static double[] one = new double[]{1, 0};
    static double[] i = new double[]{0, 1};
    static double[] zero = new double[]{0, 0};
    ...
    static double[] conjugate(double[] z){ // a - bi
        return new double[]{z[0], -z[1]};
    }

    static double[] add(double[] z, double[] w){ // z + w
        return new double[]{z[0]+w[0], z[1]+w[1]};
    }

    static double[] subtract(double[] z, double[] w){ // z - w
        return new double[]{z[0]-w[0], z[1]-w[1]};
    }

    static double[] multiply(double[] z, double[] w){ // z*w = (a+bi)(c+di) = (ac-bd) + (ad+bc)i
        return new double[]{z[0]*w[0]-z[1]*w[1], z[0]*w[1]+z[1]*w[0]};
    }

    static double[] divide(double[] z, double[] w){ // z / w = (z*conj(w))/(w*conj(w)) = (z*conj(w))/(|w|^2)
        if(w[0] == 0 && w[1] == 0){
    ...
    static double[] exp(double[] z){ // e^z := e^Rez(cosImz + sinImz)
        return new double[]{Math.exp(z[0])*Math.cos(z[1]), Math.exp(z[0])*Math.sin(z[1])};
    }

    static double[] log(double[] z){ // logz := log|z| + iarg(z)
        return new double[]{Math.log(Math.hypot(z[0], z[1])), Math.atan2(z[1], z[0])};
    }

    static double[] power(double[] z, double[] w){ // z^w := e^logzw
